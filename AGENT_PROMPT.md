...
3. Read `progress.md` and recent `git log --oneline -10` to understand current state
...
6. If all features in `feature_list.json` have `"passes": true`, output `ALL_FEATURES_DONE` and stop
7. If you sense context is getting large (many tool calls, long conversation), commit your progress and exit cleanly — the outer loop will restart you with fresh context

## Rules

- One feature per context window — don't try to do everything at once
...
- If tests fail after 3 attempts, skip the feature (leave `"passes": false`) and move to the next one
